/**
 * Alternative script using Bible Gateway or other free APIs
 * This version uses the free Bible.com API
 */

const fs = require('fs');
const path = require('path');

// Alternative: Use bible-api.com (free, no key required)
...
async function fetchPsalmFromBibleAPI(psalmNumber) {
  try {
    const response = await fetch(`${BIBLE_API_BASE}/psalms/${psalmNumber}?translation=kjv`);
...
function parseVersesFromAPI(apiData, psalmNumber) {
  const verses = [];
  
  if (apiData && apiData.verses) {
    apiData.verses.forEach(verse => {
      verses.push({
        id: `psalm-${psalmNumber}-${verse.verse}`,
...
async function generateAllPsalmsFromAPI() {
  console.log('Generating all Psalms data from Bible API...');
  const psalms = [];
  ...
  for (let i = 1; i <= 150; i++) {
    ...
    const apiData = await fetchPsalmFromBibleAPI(i);
    if (apiData) {
      const verses = parseVersesFromAPI(apiData, i);
      if (verses.length > 0) {
        psalms.push({
          number: i,
...
if (require.main === module) {
  generateAllPsalmsFromAPI().catch(console.error);
}

module.exports = { generateAllPsalmsFromAPI, fetchPsalmFromBibleAPI, parseVersesFromAPI };
